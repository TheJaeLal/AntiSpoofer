...
try:
	filter_file = open('filter.conf','r')
except FileNotFoundError:
	print('***Error: Missing \'filter.conf\' file***')
	exit()

ip_filter = filter_file.read().split('\n')
...
for line in ip_filter:
	if line.strip() == "#BlackList":
		isBlackList = True
		isWhiteList = False
	elif line.strip() == "#WhiteList":	
		isWhiteList = True
		isBlackList = False
	elif line.strip() == "":
		continue	
...
while True:
	...
	if ip_protocol == 'IPv4':
		src_ip,dest_ip = parse_packet(ip_packet)	
		if src_ip == my_ip:
			print('Sent from My Machine')
			continue	
		elif src_ip in black_list:
			print('Request from \'{}\' has been blocked'.format(src_ip))		
		elif src_ip in white_list:
			print('Request from \'{}\' has been granted'.format(src_ip))		
		else:
			print('\'{}\': is neither blacklisted or whitelisted'.format(src_ip))
